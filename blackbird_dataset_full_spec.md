...
#### 3.6 Interruption Handling and Resume (New)
- **State Files:** Sync operations create a state file (e.g., `.blackbird/operation_sync_<timestamp>.json`) before starting.
- **Content:** This file tracks the status of each file to be synced using a hash of its symbolic path (e.g., `pending`, `done`, `failed: <reason>`).
- **Updates:** The file is updated periodically during the sync process.
- **Resume:** If a sync is interrupted, you can use the `blackbird resume <state_file.json>` command to continue the operation. It will:
    - Load the state file.
    - Check the local filesystem (in the target location specified in the state file) for files marked `pending` or `failed`.
    - Skip files that already exist with the correct size.
    - Attempt to download the remaining required files.
    - Update the state file as progress is made.
- **Completion:** The state file is automatically deleted upon successful completion of the sync or resume operation.
...
# Resume an interrupted sync operation (New)
blackbird resume /path/to/dataset/.blackbird/operation_sync_1678886400.json \
    [--dataset-path /path/to/dataset]
# If --dataset-path is omitted, it's inferred if the state file is inside .blackbird
```
