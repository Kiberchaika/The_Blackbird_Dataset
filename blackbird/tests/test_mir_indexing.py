"""Tests for DatasetIndex.build() — .mir.json indexing, component counting, and sizing."""
...
# ---------------------------------------------------------------------------
# Multi-component dataset: instrumental + vocals + mir (3 components)
# Tests component counting, sizing, and aggregation.
# ---------------------------------------------------------------------------

@pytest.fixture
def multi_component_dataset(tmp_path):
    """Create a dataset with 3 component types and known sizes."""
    dataset_root = tmp_path / "multi_dataset"
    blackbird_dir = dataset_root / ".blackbird"
    blackbird_dir.mkdir(parents=True)
...
@pytest.fixture
def multi_index(multi_component_dataset):
...
class TestComponentCounts:
    ...
    def test_total_tracks(self, multi_index):
        assert len(multi_index.tracks) == 3

    def test_all_tracks_have_instrumental(self, multi_index):
        for track in multi_index.tracks.values():
            assert "instrumental" in track.files

    def test_all_tracks_have_mir(self, multi_index):
        for track in multi_index.tracks.values():
            assert "mir_analysis" in track.files

    def test_vocals_count(self, multi_index):
        """Only Album1 tracks (2) should have vocals."""
        count = sum(1 for t in multi_index.tracks.values() if "vocals" in t.files)
        assert count == 2
...
class TestComponentSizes:
    ...
    def test_total_size_positive(self, multi_index):
        assert multi_index.total_size > 0

    def test_instrumental_sizes(self, multi_index):
        for track in multi_index.tracks.values():
            if "instrumental" in track.files:
                path = track.files["instrumental"]
                assert track.file_sizes[path] in (4000, 6000)

    def test_mir_json_sizes_small(self, multi_index):
        for track in multi_index.tracks.values():
            if "mir_analysis" in track.files:
                path = track.files["mir_analysis"]
                assert track.file_sizes[path] < 100


class TestComponentAggregation:
    """Tests for aggregating component data across the index."""

    def test_component_count_by_type(self, multi_index):
    ...
    def test_component_total_size_by_type(self, multi_index):
        sizes = defaultdict(int)
        for track in multi_index.tracks.values():
            for comp_name, file_path in track.files.items():
                sizes[comp_name] += track.file_sizes[file_path]
        assert sizes["instrumental"] == 14000  # 2×4000 + 1×6000
        assert sizes["vocals"] == 6000         # 2×3000

    def test_artist_album_structure(self, multi_index):
        assert len(multi_index.album_by_artist) == 2
        assert "Artist1" in multi_index.album_by_artist
        assert "Artist2" in multi_index.album_by_artist

    def test_stats_by_location(self, multi_index):
        assert "Main" in multi_index.stats_by_location
        main_stats = multi_index.stats_by_location["Main"]
